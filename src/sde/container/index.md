...
|------|---------|---------|
| 保护对象 | 宿主机与其他租户 | 容器内容 |
| 攻击者 | 容器内代码 | 宿主机（root/hypervisor/运维） |
| 信任根 | Linux 内核 | CPU 硬件 + 厂商密钥 |
| 核心机制 | Namespace/Cgroup/Cap/Seccomp/MAC | TEE 内存加密、远程证明、密封密钥 |
| 威胁模型 | 软件漏洞利用 | 物理访问、恶意 hypervisor、冷启动攻击 |
| 性能开销 | 极低（原生性能） | 显著（加密内存访问、证明开销） |
