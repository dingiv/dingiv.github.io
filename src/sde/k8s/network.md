...
ExternalIP 允许用户手动为 Service 指定一个或多个外部 IP 地址，当外部流量到达这些 IP 时会被路由到集群内的 Service。与 LoadBalancer 不同，ExternalIP 不会自动向云厂商申请资源，IP 的分配和路由配置完全由用户自行管理。这种方式适用于已有固定 IP 资源且不想依赖云厂商负载均衡 API 的场景，但需要用户自行确保外部网络能够将流量正确路由到集群节点。

在工程实践中，ExternalIP 常见于自建机房环境，运维团队通常会结合 BGP 或静态路由将外部 IP 的流量引导到集群的边缘节点上。需要注意的是，ExternalIP 缺乏健康检查机制，如果某个节点不可达但 IP 仍然绑定在 Service 上，流量会被路由到不可用的节点。因此在生产环境中，建议配合Keepalived 或类似的高可用方案来实现 IP 的故障转移。
