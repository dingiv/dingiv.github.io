...
+ 回表：MySQL InnoDB 的二级索引叶子节点存储的是索引列值和主键值，而非完整行数据。当查询需要的列不在二级索引中时，存储引擎需要用二级索引找到的主键去聚簇索引中查找完整行，这个过程称为回表。回表的本质是额外的随机 I/O——每回表一次就是一次聚簇索引的树查找，匹配行数越多，回表开销越大。如果预估回表次数过多（通常由优化器基于统计信息判断），优化器可能直接放弃索引，选择全表扫描。因此减少回表是索引优化的核心目标之一，覆盖索引和索引下推都是围绕减少回表展开的。
