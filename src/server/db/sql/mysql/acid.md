...
四个属性的性质并不相同：A 和 D 是**工程实现**的承诺（通过日志和刷盘机制保证），I 是**并发语义**的定义（通过锁或 MVCC 实现），C 是**这一切的目的**（前三个属性共同服务的最终结果）。理解这个层次关系比记住四个名词重要——下文按 MySQL/InnoDB 的实现展开，但框架适用于所有事务型数据库。
...
## 四要素的协作全景
一个提交动作的完整链路把四个属性串起来：执行修改（隔离性保证不与其他事务冲突）→ 写 undo/redo 日志（原子性和持久性的准备）→ fsync redo log（持久性生效时刻）→ 修改缓冲池中的页（一致性在新状态成立）→ 响应客户端成功。崩溃可能发生在任何一步——恢复机制（undo 回滚未提交、redo 重放已提交）保证无论在哪一步崩溃，数据库都回到"全部已提交事务生效、全部未提交事务消失"的一致状态。

ACID 的工程价值不在于背诵定义，而在于知道每个属性由什么机制保证、妥协它换什么、什么时候必须坚持它。事务的默认 ACID 是安全网——理解网眼的形状（隔离级别、fsync 时机、约束检查时机），才知道什么情况下会漏下去。
