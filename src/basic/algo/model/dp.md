...
# 动态规划
动态规划是一种使用空间来换时间的一种典型的算法和思想，具有广泛的使用范围，动态规划的本质是使用数学中的**数列的递推公式来解决计算重复的问题**，使用空间换时间的思想，缓存计算过程中的中间表示，避免重复计算，从而实现

特别是经常用来解决一些最值问题，这些最值问题存在以下特点：
...
1. **定义状态数组**：确定状态表示，选择合适的数据结构，确定状态维度。
   ...
   即选择一个 dp 数组，定义 dp 数组中的每一个元素 `dp[i]/dfs(i)` 的意义是什么。
