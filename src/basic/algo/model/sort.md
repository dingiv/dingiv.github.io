...
## 归并排序
采用分治思想进行分割式排序，将一个大的问题转化为一个个子问题，然后再将各个子问题统一解决。有自顶向下的递归法，也有自底向上的循环，可以在小的范围时转接其他排序进行小数组优化。优化的手段可以从这几个方面考虑：1、小数组转接；2、归并预判；3、原地归并
```javascript
...
## 快速排序
使用分治思想，并进行大跨度交换，可以看做是插入排序和希尔排序的延续，同时又综合了归并排序的分治思想。优化的手段可以从这几个方面考虑：1、小数组转接；2、三取样决定分割数；3、三向分割，优化出现大部分的相同的数值作为分割数的情况。
```javascript
...
## 堆排序
堆又可以称之为优先队列，可以在数组上构造出分支结构，可以看做是使用分支结构带来的优势，将极端的元素快速选出并放置至合适的位置。是一种基于堆的选择排序。
```javascript
