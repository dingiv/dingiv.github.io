...
## 与内核设施的关系
块设备的复杂链路（页缓存 → 调度器 → 请求队列）解释了为什么 `io_uring` 和 `O_DIRECT` 在高性能场景有意义：它们绕过了部分中间层。也解释了 sendfile 为什么快：数据从页缓存直接到网络栈，不经过用户态缓冲区。字符设备的短链路解释了为什么终端和串口这类交互设备即使没有缓存也不影响体验——数据量小、延迟敏感、无复用需求。
