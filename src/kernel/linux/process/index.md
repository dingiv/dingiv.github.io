...
### fork 树与资源继承
进程树的本质是"fork 的派生历史"——每次 fork 复制的不只是代码执行的上下文，还有一族资源的引用。理解"fork 继承什么"等于理解 Unix 环境的传播机制：

| 继承项 | fork 行为 | 工程含义 |
|--------|----------|---------|
| 文件描述符 | 复制引用（共享同一 file offset） | 父子写同一 fd 会互相推进偏移——重定向在 fork 前设置才对全部子进程生效 |
| 当前工作目录/根目录 | 复制引用 | shell 中 cd 影响后续命令的原因 |
| 环境变量 | 复制副本 | fork 后修改不影响对方 |
| 信号处置 | 复制（忽略/捕获保留，默认重置的除外） | daemon 对 SIGHUP 的忽略被 worker 继承 |
| 进程组/会话 | 继承 | 信号可以按组广播到整个 fork 族 |
| 内存地址空间 | COW 复制 | 写时才真正复制——fork 快的本质原因 |
| 锁（fcntl/pthread） | 复制引用但语义分裂 | **fork 与锁的经典陷阱**——见下 |

fork 与多线程的锁陷阱值得单独强调：多线程程序中一个线程持锁时另一线程 fork，子进程只继承调用 fork 的那个线程——持锁的线程不存在了，但锁的状态是"已持有"。子进程若再获取这把锁将永久死锁。POSIX 的对策是 `pthread_atfork`（fork 前后在父/子进程执行回调处理锁状态），现代实践则是**多线程程序中只允许 async-signal-safe 的操作在 fork 后、exec 前执行**——fork 后立即 exec 是唯一安全的模式。

树的演化形态也是工程排查的线索：`pstree` 看到进程挂在 init 下（被收养的孤儿）、sshd 派生的 bash 再派生命令（登录会话的传播链）、容器运行时的 shim 进程树（runc → containerd-shim → 容器 init）——进程树的形态直接反映系统的服务架构。

### 会话与终端的关联
会话（session）的完整语义围绕控制终端（controlling terminal）：登录时 login 进程创建新会话并设置终端为控制终端，shell 成为会话首进程；shell 的每个前台作业是一个进程组，终端的输入送给前台组，终端产生的信号（Ctrl-C 的 SIGINT、Ctrl-Z 的 SIGTSTP、Ctrl-\ 的 SIGQUIT）发送给前台组全体。这个模型解释了：为什么后台进程读终端会被暂停（SIGTTIN）、为什么 nohup 通过忽略 SIGHUP 让进程脱离终端生命周期、为什么 daemon 的标准姿势是 setsid（新会话，脱离控制终端）+ 两次 fork（避免再获得终端）。
