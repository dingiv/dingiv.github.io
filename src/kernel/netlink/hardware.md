...
### WiFi 2.4G/5G
WiFi 是局域网组网中唯一的"无线以太网"——它在 MAC 层与以太网同源（802.11 的帧格式与 802.3 高度相似），在物理层使用无线电。2.4GHz 和 5GHz 两个频段的取舍是 WiFi 选型的核心问题：2.4GHz 穿透性强（波长长，穿墙衰减小）但信道拥挤（只有 3 个不重叠信道，蓝牙、微波炉、无线键鼠都在这个频段）；5GHz 带宽大（80/160MHz 信道，可达 Gbps 级）但穿透性弱（一堵墙可能衰减一半信号）。

WiFi 的工程特性决定了它不适合作"骨干组网"而适合"接入组网"：延迟抖动大（空口竞争、重传）、多设备共享带宽（AP 的时分复用）、易受环境干扰。工程实践中 WiFi 的角色是"最后十米"——骨干走有线以太网，终端设备（笔记本、手机、IoT 传感器）通过 WiFi 接入。需要确定性低延迟的场景（工业控制、实时音视频回传）应避免 WiFi。

### 蓝牙
蓝牙是短距离（10m 级）、低功耗的个人局域网（PAN）技术。与 WiFi 的本质区别在拓扑和应用模型：WiFi 是"多设备接入一个 AP"的星型网络，蓝牙经典版是"一个主机 + 最多 7 个从设备"的微微网（piconet），BLE（低功耗蓝牙）则是面向周期性小数据（传感器读数、心率）的广播/连接模式。

蓝牙的工程定位是"设备到设备"而非"网络"——键鼠、耳机、穿戴设备、智能家居传感器。BLE 的功耗优势（纽扣电池供电数年）使其成为 IoT 传感器的首选无线方案，但带宽（2Mbps BLE 5.x）和距离限制了它承担"组网"角色的可能。蓝牙 Mesh 试图把蓝牙扩展为组网技术，但生态成熟度与 Zigbee/Thread 相比仍有差距。

### Fiber Channel
光纤通道（Fiber Channel, FC）是存储专用网络（SAN）的传统霸主。它不传输 IP 报文——传输的是 SCSI 命令：服务器通过 FC 网络向存储阵列发送块级读写请求，就像访问本地磁盘一样访问远端存储。FC 的协议栈自成一派（FC-0 到 FC-4 层），物理层使用光纤，速率从 8GFC 到 64GFC。

FC 的核心优势是确定性：专用交换网络、无拥塞设计（buffer-to-buffer credit 流控）、低延迟（μs 级）。代价是专有生态——FC 交换机（Brocade/Cisco MDS）、FC HBA 卡、光模块都是专有硬件，价格远超以太网。

FC 的现状是被以太网侵蚀。iSCSI（SCSI over TCP/IP）以更低的成本提供类似功能；NVMe-oF（NVMe over Fabrics）在 RDMA 以太网（RoCEv2）上提供比 FC 更低的延迟和更高的 IOPS。新一代存储网络的主流选择已经从"FC 还是 iSCSI"变为"RoCEv2 还是 TCP"——FC 仍然存在于金融、电信等对稳定性和存量投资敏感的行业，但新部署中的份额持续下降。
