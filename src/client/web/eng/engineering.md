# 工程化搭建
项目搭建解决的是"项目骨架长什么样"的问题，工程化搭建则解决"团队怎么高效协作和持续交付"的问题。一个设计良好的工程化体系应该覆盖代码规范、环境管理、版本控制和持续集成，让 20 个人的团队像 1 个人一样高效协作。
...
在部署策略上，前端项目通常使用 Docker 容器化部署或直接将静态文件上传到 CDN/对象存储（如 AWS S3、阿里云 OSS）。Docker 多阶段构建（Multi-stage Build）是容器化部署的标准模式：第一阶段使用 Node 镜像执行 `pnpm install` 和 `vite build`，第二阶段使用轻量级的 Nginx 镜像只复制构建产物，最终的生产镜像只包含 Nginx 和静态文件，体积极小。
...
# 只有 package.json 或 pnpm-lock.yaml 变化时才重新安装依赖
...
Dockerfile 编写中有几个工程细节值得注意。依赖文件和源码分两步 `COPY` 是为了利用 Docker 的层缓存机制——`pnpm install` 是最耗时的步骤，只有 `package.json` 和 `pnpm-lock.yaml` 变化时才需要重新执行，源码的日常改动不会触发依赖重装。`--frozen-lockfile` 参数确保安装行为严格遵循 lock 文件，避免因版本漂移导致的构建不一致。构建指令中应尽量使用非 root 用户，避免容器以 root 权限运行带来的安全风险。

Nginx 配置文件通常需要处理三项工作：SPA 路由的 fallback（所有未匹配的路径都返回 `index.html`，由前端路由接管）、静态资源的缓存策略、以及 Gzip/Brotli 压缩。
...
Docker 镜像的日常管理方面，给镜像打标签（Tag）是版本追溯的基础。推荐使用语义化版本号（`v1.2.3`）配合 Git commit SHA 短哈希（`v1.2.3-a1b2c3d`）的双重标签，前者用于回滚和发布管理，后者用于精确定位构建来源。镜像仓库（如 Docker Hub、阿里云 ACR、GitHub Container Registry）的清理策略也需要规划——前端镜像迭代频繁，不及时清理旧版本会快速耗尽存储空间。可以配合 CI 流水线在每次新版本推送后自动清理该分支的旧标签镜像。
