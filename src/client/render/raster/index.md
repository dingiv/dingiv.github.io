...
### 应用程序阶段
- 应用程序阶段（Application Stage）运行于 CPU，由开发者控制，负责准备渲染所需的数据和逻辑。
- 任务：
...
### 顶点着色
- 顶点着色（Vertex Shading）是 GPU 上运行的可编程阶段，处理每个顶点的几何变换和属性计算。注意，该命名极具误导性，其更贴切的命令应当是**投影变换**。
- 任务：
...
### 片段着色
- 片段着色（Fragment Shading）为每个片段计算最终颜色，决定像素外观。该步骤利用光照模型，模拟计算场景中的灯光打到模型表面时，在相机的视角应当看见的颜色数据。是整个图形渲染的核心步骤。
- 任务：
