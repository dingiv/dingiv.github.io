...
## 嵌入式 HMI
嵌入式人机交互界面。是客户端界面开发，但是属于嵌入式开发的一部分，使用较为保守和封闭的技术栈，主要由 C++/Qt 主导，部分低性能的场景使用 C 即可，目前 Rust 和 Dart 也在逐步进入该领域。短期内不会有太大的变化要点。

编写这些程序的难度主要来源于 C++/Qt 本身，在 UI 的设计和编写速度上不如高性能平台的框架快速现代，风格老旧，缺乏进步动力。但是，胜在生态完整成熟，性能要求高，使用 Qt 编写的图形依然能够具有较高的审美上限，酷炫界面依然能够做到。

## 命令行终端客户端
该领域目前是 Rust 的专场。

命令行程序可能是 GUI 的对立面，但在现代的命令终端程序中，相当一部分已经支持较为复杂的展示逻辑，通过字符拼接，从而呈现出优美的 UI 效果，称之为 TUI 程序。以前这个领域是 C 和 C++ 的天下，后来 Python 崛起成为脚本语言的首选，现在 Rust 正在悄悄接管这个领域。
