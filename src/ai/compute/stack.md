...
## 算力软件栈
从上层应用到硬件的计算资源调用链路如下，同时注意 GPU 计算为异构计算，包含运行在 CPU 上的管控面代码和运行在 GPU 上的数据面代码。

| 层次              | CPU 管控面                   | GPU 数据面 |
| ----------------- | ---------------------------- | ---------- |
| 大模型应用层      | 提示词与记忆管理             | NA         |
| 模型算法层        | 实现具体的 AI 模型算法       | NA         |
| AI 基架层         | 按照模型结构运行 AI 模型     | NA         |
| 加速计算标准接口  | 规定跨硬件厂商的加速计算规范 | NA         |
| HAL 层 + 系统调用 | 实现具体的 AI 模型算法       | NA         |
| 内核驱动层        | 实现具体的 AI 模型算法       | NA         |
| 硬件层            | 实现具体的 AI 模型算法       | NA         |
