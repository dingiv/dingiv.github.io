...
AVX 将寄存器宽度从 128 位扩展到 256 位，使得一条指令可以处理 8 个 float（32-bit）或 4 个 double（64-bit）。AVX2 进一步增加了整数 SIMD 操作，使得整数和浮点都可用向量指令处理。

AVX-512 将寄存器宽度扩展到 512 位，单寄存器可容纳 16 个 FP32 或 8 个 FP64。以矩阵乘法为例，CPU 用 AVX-512 一次运算 16 个乘法和 16 个加法，相比无 SIMD 的标量计算理论加速 16 倍。实际加速比受限于内存带宽——数据在 L3 缓存中时 AVX-512 效率非常高，在 DDR 内存中时 CPU 等待数据的时间远大于计算时间，SIMD 加速几乎看不出来。

AVX-512 的缺点是功耗和发热——512 位寄存器和执行单元占用大量芯片面积，全速运行时 CPU 温度显著上升。Intel 从 12 代酷睿起在消费级 CPU（P-core）上通过熔断禁用了 AVX-512，只在服务器级 Xeon 上保留。AMD 的 Zen 4 通过两个 256 位单元拼接实现 AVX-512，功耗控制更好。常用 AVX-512 子集：F（基础）、CD（冲突检测）、BW（字节和字操作）、DQ（双字和四字操作）、VL（可变长度）、IFMA（整数 FMA）、VBMI（向量位操作）。
...
BLAS（Basic Linear Algebra Subprograms）是线性代数的标准 API 规范，不是硬件指令。它定义接口，不定义实现——Level 1 为向量运算（如 `daxpy`：y = a*x + y）、Level 2 为矩阵-向量运算（如 `dgemv`）、Level 3 为矩阵-矩阵运算（如 `dgemm`）。

各大厂商和社区提供了各自的优化实现：

| 库               | 平台          | 特点                                      |
| ---------------- | ------------- | ----------------------------------------- |
| Intel MKL        | Intel CPU     | 优化最深，AVX-512 hand-tuned kernel       |
| AOCL (AMD)       | AMD CPU       | Zen 架构优化，基于 BLIS+SIMD intrinsics   |
| OpenBLAS         | 开源，跨平台  | 运行时检测 SIMD 指令集，自动选最优 kernel |
| Apple Accelerate | Apple Silicon | 通过 AMX 协处理器加速矩阵乘法             |
| BLIS             | 开源，跨平台  | 模块化 micro-kernel，适配新架构只改几行   |

BLAS 和 SIMD 的关系是分层协作的。BLAS 定义"做什么"（矩阵乘法的接口），SIMD 决定"怎么快"（用 AVX-512 一条指令算 16 个数）。OpenBLAS 的 `dgemm` 内部用 AVX-512 kernel 实现——这个 kernel 用 512 位寄存器指令手写汇编，把 CPU 的矩阵乘法性能推到理论极限的 80-90%。NumPy、PyTorch CPU 推理、candle CPU 后端、MATLAB 的底层都是 BLAS → SIMD 这条通路。这也是 CPU 推理引擎能跑得动矩阵乘法的根本原因。

PyTorch 的 CPU 后端默认使用 Intel MKL（Intel CPU）或 OpenBLAS（其他 CPU）。在 AVX-512 支持的 CPU 上，FP32 矩阵乘法性能可达 500 GFLOPS，约为 A100 GPU（20 TFLOPS）的 2.5%，对于小模型推理已足够。

**AMD CPU 的选择**：首选 **AOCL**（AMD Optimized CPU Libraries）——AMD 官方推出的对标 MKL 的优化库，BLAS 部分基于 BLIS 框架，为 Zen 3/Zen 4/Zen 5 的 AVX2 和 AVX-512 做了 hand-tuned kernel。其次选 **OpenBLAS**——在 AMD CPU 上自动检测 Zen 架构并启用对应优化路径，性能接近 AOCL 且安装更简单（`apt install libopenblas-dev`）。

Intel MKL 在历史上对非 Intel CPU 存在**故意降速**的问题——检测到 `GenuineIntel` 之外的 CPUID 时会走最慢的通用路径，性能只有正常路径的 30-50%。这个行为在 2020 年被社区发现，Intel 后来修改了 MKL 的调度逻辑但二线 CPU 仍然没有 Intel 自家 CPU 的优化深度。在 AMD 上使用 MKL 可以通过 `export MKL_DEBUG_CPU_TYPE=5` 强制模拟 Intel CPU 启用 AVX-512 路径，但不如直接使用 AOCL 或 OpenBLAS 稳妥。
