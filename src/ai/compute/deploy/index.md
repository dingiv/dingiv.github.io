...
从零开始的流程：[跑起模型](run) → 选择合适的部署工具和配置。深入各环节：[GPU 硬件选型](gpu-ai) → [模型量化](quantization) → [多卡推理](multi-gpu) → [推理优化](optimization) → [服务部署](serving)。模型行为优化：[微调](finetune) 改变模型的领域知识和输出风格。生产运维：[LLMOps](llmops) 负责评估、监控和成本控制。
...
量化原理和格式选型见[模型量化](quantization)，多 GPU 策略见[多卡推理](multi-gpu)，推理加速技术见[推理优化](optimization)，GPU 硬件选型见 [GPU AI 部署参考](gpu-ai)。
...
引擎的详细对比见[推理引擎](/ai/app/engine/)，工具生态和启动命令参考见[本地部署](deploy)。
