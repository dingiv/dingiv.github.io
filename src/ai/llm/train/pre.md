...
预训练是资源密集型任务，对于大多数团队来说，基于开源模型进行增量预训练或领域适配是更现实的选择。增量预训练使用领域特定数据（如医疗文本、法律文档）继续训练预训练模型，能显著提升模型在该领域的表现，而成本远低于从零训练。关键是要使用较小的学习率（通常是原始预训练学习率的 10-20%），避免破坏预训练学到的通用知识。

数据评估是容易被忽视的环节。在投入大量计算资源之前，应该在小规模数据集上训练小型模型，评估数据质量。如果小模型无法从数据中学到任何有用的模式，那么大模型也不会更好。常用的评估指标包括：困惑度（衡量模型预测能力）、下游任务性能（在问答、分类等任务上的表现）、人工评估（模型输出的质量）。

训练监控同样重要。除了损失曲线，还应该监控梯度范数（检测梯度爆炸）、参数更新幅度（学习率是否合适）、GPU 利用率（是否存在瓶颈）。设置自动化脚本，在异常情况（如损失突增）时暂停训练并发送警报，能避免浪费计算资源。

最后，预训练的最终目标不是在训练集上达到最低损失，而是获得能够泛化到多样化任务的模型。因此，需要在训练过程中定期在验证集和下游任务上评估，选择泛化性能最好的检查点，而不是训练损失最低的检查点。这也是为什么 GPT-4、Claude 等模型的训练需要数月——大量时间用于评估和选择最佳模型，而非纯粹的训练。
