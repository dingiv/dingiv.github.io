...
# 媒体生成
文本到图像、文本到视频的生成是 AI 领域最直观也最引人注目的能力。Stable Diffusion、DALL-E、Sora 等模型能够根据文字描述生成逼真的图像和视频，正在改变设计、影视、广告等创意产业的创作流程。
...
### 采样器
采样器（Sampler）决定了去噪过程中每一步的具体算法。DDIM 是最基础的采样器，速度较快但细节表现一般。Euler 和 Euler a 是最常用的采样器，Euler 是确定性的（相同参数生成相同结果），Euler a 带有随机性（每次生成略有不同），两者在速度和质量上都比较均衡。DPM++ 2M 是近年推出的高质量采样器，在 20 步左右就能达到很好的效果，是目前 SDXL 的推荐选择。LCM（Latent Consistency Models）是加速技术，能将步数压缩到 4-8 步而保持可接受的质量，适合对生成速度要求高的场景。
...
## 视频生成
视频生成是图像生成在时间维度的扩展，面临额外的一致性挑战——连续帧之间需要保持视觉一致性（人物外观、背景、光影不能突然变化）。Sora（OpenAI）和 Kling（快手）代表了当前视频生成的最高水平，能根据文字描述生成长达 1-2 分钟的视频。

视频生成的核心技术路线有三种。图像+运动：先生成关键帧图像，再用运动模型生成帧间过渡（如 SVD，Stable Video Diffusion）。这种方式的优势是帧间一致性好（因为关键帧是固定的），但运动范围和复杂度受限。时空扩散：在 U-Net 中引入时间维度的注意力层，同时学习空间和时间特征。Sora 采用了这种路线，使用 DiT（Diffusion Transformer）架构替代 U-Net，支持更长的序列和更复杂的运动。自回归生成：将视频帧作为序列，逐帧自回归生成。这种方式类似于语言模型的生成方式，但计算成本极高。

视频生成的工程挑战包括：显存需求（视频是 3D 数据，比图像大一到两个数量级）、训练数据（高质量的视频-文本配对数据远少于图像数据）、评估标准（图像质量可以量化，视频还需要评估时序一致性和运动自然度）。
...
ComfyUI 的优势在于灵活性和性能。它的节点系统支持自定义扩展，社区贡献了数百个插件（如 AnimateDiff 动画生成、IP-Adapter 风格迁移、InstantID 人脸定制）。ComfyUI 的显存管理比 WebUI 更高效，通过模型自动卸载和按需加载，在 6-8GB 显存的显卡上也能运行 SDXL。工作流可以导出为 JSON 文件分享，其他人导入后直接运行，降低了复杂 pipeline 的使用门槛。
