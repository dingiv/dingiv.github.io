...
## 微调
当通用模型无法满足特定领域需求时，微调是必要的手段。LoRA (Low-Rank Adaptation) 是最流行的参数高效微调方法，它冻结原始模型权重，在注意力层的 W_q、W_v 矩阵旁添加低秩矩阵 A 和 B，只训练这两个矩阵。假设原始权重是 d×d，LoRA 添加两个 d×r 的矩阵（r 通常取 8-64），参数量从 d² 降低到 2dr。7B 模型用 LoRA 微调只需要几十 MB 的额外参数，而且可以快速切换不同领域的 LoRA 权重。

QLoRA 将 LoRA 推进一步，它将基础模型量化到 4bit，然后在量化权重上训练 LoRA 适配器。通过 NormalFloat 4bit 数据类型、双重量化、分页优化器等技术，QLoRA 可以在 48GB 显存的 GPU 上微调 65B 模型。这降低了微调的硬件门槛，让个人开发者也能训练大模型。

微调框架方面，Hugging Face PEFT 库提供了 LoRA/QLoRA 的统一接口，配合 Transformers 和 Accelerate 可以快速启动训练。Axolotl 是开箱即用的微调工具，通过 YAML 配置文件指定模型、数据集、超参数，自动处理训练流程。LLaMA-Factory 提供了 Web UI，支持多种微调方法（LoRA、QLoRA、Full）和训练任务（指令微调、预训练、DPO），对新手友好。

选择是否微调需要评估 ROI。RAG、Prompt Engineering 等无模型修改的方案成本低、见效快，应该优先尝试。微调适合的场景：需要改变模型的输出风格（如特定语气、格式）、需要模型掌握大量领域知识（无法通过检索覆盖）、需要优化特定任务的表现（如代码生成、医学问答）。微调的效果很大程度上取决于训练数据质量，精心构造的几千条指令数据可能比百万条噪声数据更有效。
