...
**BF16 支持是硬门槛。** 目前绝大多数开源大模型（Llama 3/4、Qwen 2.5/3.6、DeepSeek-V3）的预训练权重均为 BF16 格式。BF16 的数值动态范围与 FP32 相同（8 位指数），在 Attention 的 Softmax 阶段不会发生数值溢出。FP16 的指数位只有 5 位，动态范围窄，处理大模型的长序列 Attention 时极易出现 NaN 或乱码。NVIDIA 从 Ampere 架构（RTX 30 系列、A100）开始才硬件支持 BF16。Turing（RTX 20 系列）和 Volta（V100）都不支持——强行加载只能退化为 FP32（显存翻倍、速度减半）或面临数值不稳定。

**FlashAttention-2 是长上下文的基石。** FA2 通过分块计算和重排内存访问模式，将注意力计算的显存带宽利用率从 20% 提升到 80%+。不支持 FA2 的卡在 8K+ 上下文时注意力计算可能占推理时间的 40-50%。FA2 需要 sm_80（Ampere）及以上的 CUDA 算力。Turing（sm_75）和 Volta（sm_70）只能运行部分兼容的 FA1，性能和稳定性大打折扣。

**软件生态逐步放弃旧架构。** vLLM、SGLang、TensorRT-LLM 在编译优化时默认只对 sm_80+ 提供完整优化路径。FlashAttention-3、FP8 量化等最新技术完全不支持 Maxwell/Pascal/Volta 架构。
...
| Ampere | 3090, A6000, A100 | sm_80/86 | 是 | 是 | 部分 | 本地部署的架构起点 |
| Ada | 4090, RTX 6000 Ada | sm_89 | 是 | 是 | 是 | FP8 native，当前消费级最优 |
