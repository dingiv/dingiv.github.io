...
**解析**：将 PDF、Word、HTML 等格式转换为结构化文本，保留标题层级、表格结构、代码块等语义信息。不同格式的解析策略和工具选择在[文档解析](parsing)中详细讨论。

**清洗**：去除格式噪声、修复编码问题、统一术语、去重。清洗规则需要针对具体数据源定制，通用规则覆盖 80% 的情况，剩余 20% 需要人工检查和调整。

**分块**：将长文档切分为适合检索或训练的小片段。分块策略在[数据处理](processing)中详细讨论，关键是在粒度和上下文完整性之间找到平衡。
