...
public void processOrder(Order order) {
    if (order.getItems().get(0).getProduct().getStock() > 0) {
        order.getItems().get(0).getProduct().setStock(
            order.getItems().get(0).getProduct().getStock() - 1
        );
        order.setStatus("PROCESSING");
        order.getCustomer().getEmail().send("Order confirmed");
    }
}

// 重构后：遵循职责分配原则
public void processOrder(Order order) {
    order.confirm();  // Order 内部封装所有确认逻辑
}
...
class Order {
    ...
    private void notifyCustomer() {
        customer.notifyOrderConfirmed(this);
    }
}
```
