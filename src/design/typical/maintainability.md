...
可读性衡量的是理解代码的成本，它由几个具体因素决定。命名是第一位的——`processData` 和 `normalizeUserInputForValidation` 传达的信息量完全不同，前者要求读者继续往下读，后者把意图写在了名字里。控制流复杂度是第二位的——圈复杂度超过 10 的函数，读者需要在大脑中维护的路径分支就超过了工作记忆的容量，必须借助纸笔才能追踪。注释是第三位的——好注释解释"为什么"（业务约束、历史决策），坏注释复述"是什么"（`i++ 自增`），最坏的注释是撒谎的注释——代码改过而注释没改。
