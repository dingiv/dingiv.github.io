...
| 模式名称     | 核心思想                             | 典型应用场景                         | 现代方案                   |
| ------------ | ------------------------------------ | ------------------------------------ | -------------------------- |
| **单例**     | 保证一个类仅有一个实例               | 配置管理器、日志记录器、数据库连接池 | 依赖注入容器、状态管理容器 |
| **工厂方法** | 定义创建对象的接口，由子类决定实例化 | 框架扩展点、插件系统                 | 工厂函数                   |
...
| 模式           | 备注                                                                                                                                        |
| -------------- | ------------------------------------------------------------------------------------------------------------------------------------------- |
| **单例模式**   | 单例容易导致数据竞争，但是部分场景，单例依旧是必要实现方式，不过需要注意资源访问控制                                                        |
| **组合模式**   | React/Vue 的组件树是组合模式的应用，配合函数式组件和 hooks，实现方式比传统的 OOP 更加简洁。                                                 |
| **观察者模式** | Vue 和 React 的响应式系统本质是观察者模式的自动化实现，但开发者无需手动维护订阅列表。Signals（如 Preact Solid）将这一模式进一步推向声明式。 |
| **构造器模式** | 数据类（data class）、命名参数、解构赋值等语言特性使得构建复杂对象不再需要繁琐的 Builder 类。                                               |
