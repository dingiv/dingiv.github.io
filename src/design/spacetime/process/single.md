...
- **Controller（控制器）**：处理用户输入，更新模型，选择适当的视图进行显示，在这一层中，运行着程序的 while(1) 循环，以监听用户触发的事件或者请求；
...
### MVC 的进化
MVC 的缺点是 controller 的体积较大，逻辑在该层沉积；并且 View 层和 Controller 层存在一个的循环依赖的问题，这在现代程序中被认为是不健康的表现。

由此 MVC 也在不断地演进，MVP 和 MVVM 架构是在 MVC 的架构上微调，使得 controller 层完全不再感知 view，并引入了单向依赖和响应式能力。

> 虽然结构发生了变化，但是人们还是习惯叫 MVC，最终 MVC 也逐渐和 MVP，MVVM 混淆在一起，MVC 正式成为了*三层单向依赖架构*程序的代名词。Controller 层的定义更是发生前移，取而代之的是中间 Service 层。
...
MVVM 引入了响应式编程的思想，其实现方式就是在 Controller 层添加一个回调函数和消息队列机制，当上游的层，特别是 Model 变化的时候，Model 可以触发一个事件，被 Controller 层中的事件循环监听到，Controller 从队列中取出消息，发现是一个事件，而不是用户的信息，则运行对应的回调函数触发视图的更新逻辑。

响应式编程的引入了三个机制：
1. 反向调用，Model 无需依赖 View 或者 Service 层的代码就能够反向调用位于下游的函数；从程序的架构上来看，依然保持着 View -> Service -> Model 依赖顺序；
2. 异步能力，Model 变动时，可以选择异步触发事件，而不是直接调用同步的回调函数，从而实现异步能力；
3. 自动依赖更新，对于后台 Server 程序来说，View 层不需要经常变动，但是对于前端的 GUI 程序来说，View 几乎每一步都在变动，频繁地手动更新 View 层的状态那就显得程序非常繁琐和啰嗦，因此使用异步调用，在 Model 变动的时候自动更新视图的状态，将极大地简化 GUI 程序的开发。
